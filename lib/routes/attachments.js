...
module.exports = function(app) {
  ...
  app.put('/_design/:id/:attachment(*)', parseRawBody, function (req, res, next) {
    putAttachment(req.params.db, '_design/' + req.params.id, req, res);
  });

  app.put('/:id/:attachment(*)', parseRawBody, function (req, res, next) {
  ...
  app.get('/_design/:id/:attachment(*)', function (req, res, next) {
    getAttachment('_design/' + req.params.id, req, res);
  });

  app.get('/:id/:attachment(*)', function (req, res, next) {
    // Be careful not to catch normal design docs or local docs
    if (req.params.id === '_design' || req.params.id === '_local') {
      return next();
    }
  ...
  app.delete('/_design/:id/:attachment(*)', function (req, res, next) {
    deleteAttachment('_design/' + req.params.id, req, res);
  });

  app.delete('/:id/:attachment(*)', function (req, res, next) {
    // Be careful not to catch normal design docs or local docs
    if (req.params.id === '_design' || req.params.id === '_local') {
      return next();
    }
